...
import com.mapreduce.mappers.SchoolToStudentMapper;
import com.mapreduce.reducers.TopStudentReducer;

public class SchoolToStudentChain extends Configured implements Tool {
	...
	public int run(String[] arg0) throws Exception {
		...
		fs.delete(new Path(arg0[1] + "/B_HighestInSchoolAnalysis"), true);
		Job job = new Job(conf, "HighestInSchoolsAggregation");
		job.setJarByClass(MainDriver.class);

		job.setOutputKeyClass(Text.class);
		job.setOutputValueClass(Text.class);

		job.setMapperClass(SchoolToStudentMapper.class);
		job.setCombinerClass(TotalMarksCombiner.class);
		job.setReducerClass(TopStudentReducer.class);

		FileInputFormat.setInputPaths(job, new Path(arg0[0]));
		FileOutputFormat.setOutputPath(job, new Path(arg0[1] + "/B_HighestInSchoolAnalysis"));
